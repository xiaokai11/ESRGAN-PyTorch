...
def validate(model: nn.Module,
    ...
    Args:
        model (nn.Module): generator model in adversarial networks对抗性网络中的生成器
        data_prefetcher (CUDAPrefetcher): test dataset iterator测试数据集迭代器
        epoch (int): number of test epochs during training of the adversarial network
        writer (SummaryWriter): log file management function日志文件管理
        ...
        mode (str): test validation dataset accuracy or test dataset accuracy测试验证数据集的准确性或测试数据集的准确性
    ...
    # Put the adversarial network model in validation mode将对抗网络模型之于验证模式
    model.eval()
